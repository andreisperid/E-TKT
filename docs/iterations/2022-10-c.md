...
2. the press must be aligned to the middle of the tooth;
3. and of course, the pressing needs to happen with the right force;
...
Also, depending on the servo used, the distance from the servo to the daisy wheel tooth might change, and for that I'm providing different models that can be experimented. Or, since the STEP files are open, anyone can modify it to match their needs.
...
For addressing that (and the skewed / italic effect mentioned previously), I have created a calibration procedure that have two main moments:
